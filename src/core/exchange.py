...
class HyperliquidClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # Create a separate Info client for spot queries (without perp_dexs)
            # This is needed because the km perp_dexs can cause slow spot_meta() calls
            self._spot_info = Info(base_url=base_url, skip_ws=True) if perp_dexs else self._info
    ...
    async def place_orders_batch(self, requests: List[OrderRequest]) -> List[Optional[Order]]:
        ...
        # Rate limit: bulk_orders counts as 1 API call regardless of orders in batch
        # Hyperliquid allows up to 40 orders per bulk call
        await self._rate_limiter.acquire(weight=1)
        self._order_latency.start()
        ...
        try:
            ...
            # Debug: Log order count for HIP-3 (avoid logging full JSON to prevent loguru colorizer issues)
            if requests and requests[0].symbol.upper() == "US500":
                status_count = len(result.get("response", {}).get("data", {}).get("statuses", []))
                logger.debug(f"HIP-3 order batch: {status_count} statuses returned")
                ...
                if resting_count or filled_count or failed_count:
                    ...
                    for i, s in enumerate(statuses):
                        if s.get("resting"):
                            ...
                            error_msg = str(s.get("error", str(s)))
                            logger.debug(f"  Order {i}: FAILED - " + error_msg)
    ...
    async def cancel_orders_batch(self, orders: List[Tuple[str, str]]) -> int:
        ...
        try:
            ...
            # Convert symbol to API format (km:US500 for HIP-3)
            cancel_requests = []
            for symbol, order_id in orders:
    ...
    async def _refresh_account_state(self) -> None:
        ...
        try:
            ...
            if self.config.trading.symbol.upper() == "US500":
                try:
                    # Get Spot USDH balance - use _spot_info (without perp_dexs) for faster queries
                    wallet = self.config.wallet_address
                    spot_state = self._spot_info.spot_user_state(wallet)
                    balances = spot_state.get("balances", [])
                    
                    # Find USDH in balances
                    for b in balances:
                        if b.get("coin") == "USDH":
                    ...
                    # Cache the last known good USDH balance
                    if spot_usdh_total > 0:
                        self._last_usdh_total = spot_usdh_total
                        self._last_usdh_hold = spot_usdh_hold
                        logger.info(f"💰 HIP-3 USDH: ${spot_usdh_total:.2f} (hold=${spot_usdh_hold:.2f})")
                    elif hasattr(self, '_last_usdh_total') and self._last_usdh_total > 0:
                        # API returned 0 but we have a cached value - use it
                        spot_usdh_total = self._last_usdh_total
                        spot_usdh_hold = self._last_usdh_hold
                        logger.warning(f"USDH API returned $0, using cached: ${spot_usdh_total:.2f}")
                    else:
                        # Debug why we got 0 - check all balances
                        usdh_found = [b for b in balances if 'USD' in b.get('coin', '')]
                        logger.warning(f"HIP-3 USDH: $0 found. All USD-like balances: {repr(usdh_found)}")
                except Exception as e:
                    # Use cached value on error
                    if hasattr(self, '_last_usdh_total') and self._last_usdh_total > 0:
                        spot_usdh_total = self._last_usdh_total
                        spot_usdh_hold = self._last_usdh_hold
                        logger.warning(f"USDH API error, using cached ${spot_usdh_total:.2f}: {e}")
                    else:
                        logger.warning(f"Could not check Spot USDH: {e}")
